...
; +------------------+-----------------------------+----------------------------------------------------------------------+------+
; | Reserved Sectors | Fat Count * Sectors per Fat | (Directory Entry Count * 32 + Bytes per Sector - 1)/Bytes per Sector |      |
; +------------------+-----------------------------+----------------------------------------------------------------------+------+
...
ebr_volume_label:           db 'AURORA OS  '        ; 11 bytes
...
.after:
    ...
    mov ax, [bdb_sectors_per_fat]       ; ax = bdb_sectors_per_fat
    mov bl, [bdb_fat_count]             ; bl = fats
    xor bh, bh                          ; bh = 0
    mul bx                              ; ax = (fats * bdb_sectors_per_fat)
...
.root_dir_after:
    ...
    ; Read root directory (Setting parameters then calling disk_read)
...
.search_kernel:
    ...
    ; If not found, increment and try again, if all dirs were read print a kernel error
    add di, 32
    inc bx
...
disk_read:
    ...
    pop ax              ; AL = number of sectors read
